...
void main() {
  ...
  testWidgets('Multiplicar escurece onde as camadas se cruzam; Tela clareia', (
    tester,
  ) async {
    ...
    for (final (modo, esperado) in [
      ...
      expect(
        ...
        reason: 'a vermelha sozinha nao muda',
      );
      expect(
        em(px, 85, 50),
        const Color(0xFF00FF00),
        reason: 'a verde sozinha nao muda',
      );
    }
  });

  testWidgets(
    'white source follows lighten/darken and opacity rules from beta screenshots',
    (tester) async {
      for (final (mode, expected) in [
        (BlendMode.lighten, const Color(0xFFFFFFFF)),
        (BlendMode.darken, const Color(0xFFFF0000)),
        (BlendMode.colorBurn, const Color(0xFFFF0000)),
        (BlendMode.exclusion, const Color(0xFF00FFFF)),
      ]) {
        final px = await pintar(tester, [
          quadrado('white', Colors.white, const Offset(50, 50), blend: mode),
          quadrado('red', const Color(0xFFFF0000), const Offset(50, 50)),
        ]);
        expect(em(px, 50, 50), expected, reason: mode.name);
      }
      final px = await pintar(tester, [
        quadrado(
          'white',
          Colors.white,
          const Offset(50, 50),
        ).copyLayer(opacity: AnimatedDouble(.5)),
        quadrado('red', const Color(0xFFFF0000), const Offset(50, 50)),
      ]);
      final middle = em(px, 50, 50);
      expect(middle.r, 1);
      expect(middle.g, closeTo(.5, .01));
      expect(middle.b, closeTo(.5, .01));
    },
  );

  testWidgets(
    'sobre o fundo vazio, Multiplicar nao muda a camada — e o esperado',
    (tester) async {
      final px = await pintar(tester, [
        quadrado(
          'verde',
