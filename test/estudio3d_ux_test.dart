...
void main() {
  ...
  group('dominio', () {
    test('encaixar na grade e travar eixo', () {
      ...
      expect(travarEixo(const Vec3(1, 2, 3), EixoTravado.y).toString(), 'Vec3(0.0, 2.0, 0.0)');
      expect(travarEixo(const Vec3(1, 2, 3), EixoTravado.livre).toString(), 'Vec3(1.0, 2.0, 3.0)');
    });
    ...
    test('a hierarquia vem em arvore, com nivel, e a busca ignora acento', () {
      ...
      expect(
        ...
        ['Esfera:0', 'Grupo:0', 'Cubo:1', 'Cone:2', 'Orfao:0', 'Luz direcional:0', 'Camera:0'],
      );
    ...
    test('olhar para: a camera segue o objeto no tempo, e vai para o arquivo', () {
      final cam = Camera3D(
        id: 'cam',
      ...
      expect(layer.cameraAt(const Duration(seconds: 2)).target.x, closeTo(400, 1e-6),
          reason: 'o alvo e a posicao do objeto naquele instante');
      expect(layer.cameraAt(const Duration(seconds: 2)).position.z, closeTo(900, 1e-6),
      ...
      expect(solta.cameraAt(const Duration(seconds: 2)).target.x, closeTo(0, 1e-6));
  ...
  testWidgets('a tela simples: Voltar, a camera no meio, os tres pontos e as quatro ferramentas', (tester) async {
    await abrir(tester);
    ...
    expect(find.text('Camera'), findsOneWidget, reason: 'o nome da camera no ar');
  ...
  testWidgets('o + cria um cubo em dois toques e ja o seleciona', (tester) async {
  ...
  testWidgets('o + tambem cria uma luz, e a barra passa a ser a da luz', (tester) async {
    final c = await abrir(tester);
    await tester.tap(find.byKey(const ValueKey('estudio-adicionar')));
    await tester.pumpAndSettle();
  ...
  testWidgets('camera: nova camera entra no ar, e a troca e um toque', (tester) async {
    final c = await abrir(tester);
    ...
    expect(find.text('Frente'), findsOneWidget, reason: 'as vistas moram no menu da camera');
    ...
    expect(find.text(nova.name), findsWidgets, reason: 'o titulo mostra a camera no ar');

    // Com duas cameras aparece a faixa de troca rapida.
    final chipPrincipal = find.byKey(ValueKey('faixa-camera-${layer.camera.id}'));
    expect(chipPrincipal, findsOneWidget);
    await tester.tap(chipPrincipal);
    await tester.pumpAndSettle();
    layer = camadaDe(c);
    expect(layer.shots.last.cameraId, layer.camera.id, reason: 'a troca e instantanea');
    expect(tester.takeException(), isNull);
  });

  testWidgets('Selecionar so navega; Mover arrasta; Escalar escala', (tester) async {
    final c = await abrir(tester);
    await tocarNoCubo(tester);
    expect(find.text('Cubo manual'), findsOneWidget, reason: 'o toque selecionou');
    final camAntes = camadaDe(c).camera.positionAt(Duration.zero);
    double x() => camadaDe(c).scene.nodeById('cube')!.x.valueAt(Duration.zero);
    double s() => camadaDe(c).scene.nodeById('cube')!.scale.valueAt(Duration.zero);
  ...
  testWidgets('Focar enquadra o selecionado na camera', (tester) async {
    final c = await abrir(
      tester,
      camada: cena(nos: [SceneNode(id: 'cube', name: 'Cubo manual', x: AnimatedDouble(500))]),
    );
    await tocarNoCubo(tester);
    ...
    expect(cam.pointOfInterestAt(Duration.zero).x, closeTo(500, 1),
        reason: 'a camera passou a olhar o cubo');
    expect(tester.takeException(), isNull);
  });

  testWidgets('o painel da cena: buscar, esconder, renomear, duplicar, excluir', (tester) async {
    final c = await abrir(
      tester,
      camada: cena(nos: [
        SceneNode(id: 'cube', name: 'Cubo manual'),
        SceneNode(id: 'esfera', name: 'Esfera azul', kind: Element3DKind.sphere),
      ]),
    );
    ...
    // Buscar filtra.
    await tester.enterText(find.byKey(const ValueKey('estudio-busca')), 'azul');
    await tester.pumpAndSettle();
    ...
    await tester.pumpAndSettle();
    await tester.enterText(find.byKey(const ValueKey('estudio-nome')), 'Bola');
    ...
    expect(find.text('Bola copia'), findsOneWidget, reason: 'a copia esta na barra de contexto');
  ...
  testWidgets('material simples muda a cor base sem sair da tela', (tester) async {
    final c = await abrir(tester);
    await tocarNoCubo(tester);
    ...
    final regua = find.descendant(of: metalico, matching: find.byType(AmTickRuler));
    await tester.drag(regua, const Offset(-300, 0));
    await tester.pumpAndSettle();
    expect(camadaDe(c).scene.nodeById('cube')!.material.metallic, greaterThan(0.5));
    expect(tester.takeException(), isNull);
  });

  testWidgets('toque longo seleciona varios, e Agrupar poe um pai comum', (tester) async {
    final c = await abrir(
      tester,
      camada: cena(nos: [
    ...
    await tester.pumpAndSettle();
    expect(find.byKey(const ValueKey('contexto-agrupar')), findsOneWidget,
    ...
    expect(find.text(grupo.name), findsOneWidget, reason: 'o grupo fica selecionado');
    expect(tester.takeException(), isNull);
  });

  testWidgets('o modo avancado devolve vistas e comandos, e fica guardado', (tester) async {
    ...
    expect(find.text('Frente'), findsOneWidget, reason: 'as vistas voltaram a tela');
  ...
  testWidgets('a grade encaixa o arrasto em multiplos do passo', (tester) async {
    SharedPreferences.setMockInitialValues({EstudioPreferencia.kAvancado: true});
    final prefs = await SharedPreferences.getInstance();
    ...
    expect(x, isNot(0));
    expect((x / passoDeMover - (x / passoDeMover).round()).abs(), lessThan(1e-6),
        reason: 'x=$x tem de ser multiplo de $passoDeMover');
    expect(tester.takeException(), isNull);
  });

  testWidgets('Voltar fecha o Estudio e devolve a tela de baixo', (tester) async {
    tester.view.physicalSize = const Size(390, 844);
    tester.view.devicePixelRatio = 1;
    addTearDown(tester.view.reset);
    final container = ProviderContainer();
