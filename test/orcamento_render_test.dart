...
void main() {
  ...
  group('a conta', () {
    test('cada degrau da escada custa menos que o anterior', () {
      ...
      for (var i = 1; i < custos.length; i++) {
        expect(custos[i], lessThan(custos[i - 1]),
            reason: '${Qualidade3D.values[i]} nao e mais barato que ${Qualidade3D.values[i - 1]}');
      }
    });
    ...
    test('o alvo do preview respeita escala e teto por nivel', () {
      ...
      expect(alvoDoPreview(1080, 1920, ReceitaDeQualidade.alta), (largura: 608, altura: 1080));
      expect(alvoDoPreview(1080, 1920, ReceitaDeQualidade.baixa), (largura: 405, altura: 720));
      ...
      expect(alvoDoPreview(0, 0, ReceitaDeQualidade.alta), (largura: 0, altura: 0));
    });
  });
  ...
  group('a escolha pelo orcamento', () {
    ...
    test('iPhone 13, cena comum no preview: o topo, com folga', () {
      ...
      expect(r.estimativa.total, lessThanOrEqualTo(iphone13 * fracaoSeguraDoOrcamento));
    });

    test('iPhone 13, a cena extrema: mais baixo que a comum, e ainda dentro', () {
    ...
    test('a sombra efetiva nao passa do que o alvo aproveita', () {
      ...
      expect(sombraEfetiva(ReceitaDeQualidade.ultra, 0), 2048, reason: 'sem alvo ainda: a da receita');
    });
  ...
  group('a exportacao', () {
    test('4K no iPhone 13: mantem a resolucao, larga o MSAA', () {
      ...
      expect(r.estimativa.total, lessThanOrEqualTo(orcamentoGpuBytes(4 * gb) * fracaoSeguraDaExportacao));
    });
    ...
    test('so quando nem a emergencia cabe a escala desce — e desce ate caber', () {
      final r = receitaDeExportacao(
        perfil: extrema,
        orcamentoBytes: orcamentoGpuBytes(2 * gb),
        larguraPx: 7680,
