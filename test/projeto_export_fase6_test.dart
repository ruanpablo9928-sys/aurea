...
void main() {
  ...
  test('fundo da composicao: campo novo, com padrao preto e persistencia', () {
    ...
    expect(c.read(editorControllerProvider).backgroundColor, const Color(0xFF000000));
  ...
  testWidgets('exportar em dois toques: Exportar, preset 1080p', (tester) async {
    ...
    expect(find.byKey(const ValueKey('export-renderizar')), findsNothing, reason: 'ajustes finos sao Pro');
    ...
    expect(find.byKey(const ValueKey('export-preset-1080p')), findsNothing, reason: 'a folha fechou');
    expect(find.byKey(const ValueKey('editor-capture')), findsNothing, reason: 'a tela de exportacao cobre o editor');
  });

  testWidgets('⚙ Projeto: composicao editavel no Simples; guias, motion blur e paleta no Pro', (tester) async {
    ...
    expect(find.byKey(const ValueKey('projeto-areas-seguras')), findsNothing, reason: 'guias sao Pro');
    ...
    await tester.tap(find.byKey(const ValueKey('editor-settings')));
    await tester.pumpAndSettle();
    expect(find.byKey(const ValueKey('projeto-areas-seguras')), findsOneWidget);
